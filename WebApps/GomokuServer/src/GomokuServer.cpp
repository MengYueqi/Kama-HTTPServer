...
void GomokuServer::restartChessGameVsAi(const http::HttpRequest& req, http::HttpResponse* resp) {
    ...
    if (session->getValue("isLoggedIn") != "true") {
        ...
        packageResp(req.getVersion(), http::HttpResponse::k401Unauthorized, "Unauthorized", true,
    ...
    int userId = std::stoi(session->getValue("userId"));
    {
        ...
        if (aiGames_.find(userId) != aiGames_.end()) aiGames_.erase(userId);
    ...
    packageResp(req.getVersion(), http::HttpResponse::k200Ok, "OK", false, "application/json",
...
void GomokuServer::getBackendData(const http::HttpRequest& req, http::HttpResponse* resp) {
    try {
        ...
        respBody = {{"curOnline", curOnline}, {"maxOnline", maxOnline}, {"totalUser", totalUser}};
        ...
        // 错误响应
        nlohmann::json errorBody = {{"error", "Internal Server Error"}, {"message", e.what()}};
...
void GomokuServer::packageResp(const std::string& version,
                               ...
                               const std::string& statusMsg, bool close,
                               const std::string& contentType, int contentLen,
